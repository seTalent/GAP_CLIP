...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_siz, stride=patch_size) # (H/p)*(W/p) * C
...
class SwinFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=3,
                 ...
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='default', resi_connection='1conv',
                 **kwargs):
        ...
        self.Fusion_num_layers = len(Fusion_depths) #这两个地方，特征提取层和融合层
        self.Re_num_layers = len(Re_depths) #Re:Reconstruction 重建不是残差

        self.embed_dim = embed_dim
        self.ape = ape #是否添加绝对位置编码，Swin默认为False
        self.patch_norm = patch_norm #path_embedding后添加batch_normal
        self.num_features = embed_dim
        self.mlp_ratio = mlp_ratio #MLP的hidden_dim / img_embed
    ...
    def forward(self, A, B):
        ...
        x = A#[B,C, H+window_size, W+window_size] #[1,1,488,648]
        y = B
        H, W = x.shape[2:]
...
if __name__ == '__main__':
    ...
    model = SwinFusion(upscale=2, img_size=(height, width),
                   ...
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='default')
    print(model)
    print(height, width, model.flops() / 1e9)

    x = torch.randn((1, 3, 224, 224)) #image
    y = torch.randn((1, 3, 224, 224)) #flow
    x = model(x, y)
    print(x.shape)
