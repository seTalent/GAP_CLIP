...
#todo 1: 修改输入为 a photo/video of {learnable tokens}
...
class PromptLearner(nn.Module):
    def __init__(self, class_names, clip_model, args):
        super().__init__()
        n_cls = len(class_names) #C
        n_ctx = args.contexts_number #M
        ...
        if args.class_specific_contexts=='True':
            ...
            ctx_vectors = torch.empty(n_cls, n_ctx, ctx_dim, dtype=dtype) #[C, M, D]
        else:
            if args.load_and_tune_prompt_learner=='True':
